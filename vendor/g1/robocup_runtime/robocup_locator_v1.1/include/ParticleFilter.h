...
public:
	// Parameters
	double convergeTolerance = 0.4; // When the x, y, and theta ranges of all hypos are less than this value, it is considered that convergence has been achieved.
	double residualTolerance = 0.5; // If the average residual for each marker is greater than this value, the converged position is considered unreasonable.
	double maxIteration = 40;		// The maximum number of iterations.
	...
	void init(FieldDimensions fd, int minMarkerCnt = 3, double residualTolerance = 0.4, double muOffsetParam = 2.0);
