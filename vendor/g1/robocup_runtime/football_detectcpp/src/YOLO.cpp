...
void YOLO::draw(Mat& image, const vector<Detection>& output)
{
    for (int i = 0; i < output.size(); i++)
    {
        ...
        const auto& color_vec = SafeColor(class_id);
        cv::Scalar color = cv::Scalar(color_vec[0], color_vec[1], color_vec[2]);
        rectangle(image, Point(box.x, box.y), Point(box.x + box.width, box.y + box.height), color, 3);
        // Detection box text
        string class_string = SafeClassName(class_id) + ' ' + to_string(conf).substr(0, 4);
