...
使用Realsense D455相机实时获取图像并通过[yolov11](https://docs.ultralytics.com/zh/models/yolo11/)网络进行足球、球门以及球场功能线检测。现在同一个进程里还会基于 D455 的 RGB-D 帧做纯视觉里程计定位，并通过 DDS 同时发布检测结果和机器人场地位姿。检测效果如图所示:
...
### 快速开始
    ...
    执行 ./football_run.sh eth0  进行推理检测         #默认不显示，如需要显示图形请设置IMAGE_SHOWER="1"
...
### 运行
    ...
    2、推理检测+RGB-D定位
    ./football_detect ../weight/weight.engine  0 eth0 #不显示图像
    或
    ./football_detect ../weight/weight.engine  1 eth0 #显示图像

### RGB-D定位说明
    当前 `football_detect` 除了发布 `detectionresults`，还会额外发布 `rt/locationresults`，
    消息格式与 `robocup_locator_v1.1` 完全一致，因此 `Soccer_Lab` 不需要改订阅端。

    由于纯 RGB-D 里程计本身没有绝对场地锚点，首次位姿需要你按开球站位给一个初值。
    可以在运行前通过环境变量设置：

    export ROBOCUP_NET_IFACE=eth0
    export ROBOCUP_RGBD_INIT_X=0.0
    export ROBOCUP_RGBD_INIT_Y=0.0
    export ROBOCUP_RGBD_INIT_THETA_DEG=0

    另外两个保护阈值也可按需要调：

    export ROBOCUP_RGBD_MAX_STEP_M=0.30
    export ROBOCUP_RGBD_MAX_STEP_DEG=30

    例子：
    ROBOCUP_RGBD_INIT_X=-3.6 ROBOCUP_RGBD_INIT_Y=0.8 ROBOCUP_RGBD_INIT_THETA_DEG=90 \
    ./football_detect ../weight/weight.engine 0 eth0
...
- 订阅显示
    ...
    使用```cyclonedds subscribe rt/locationresults``` 可以查看 RGB-D 定位输出。
