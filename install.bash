...
configure_g1_localization_defaults() {
  ...
  # Match the field-tested robocup_deploy marker behavior by default:
  # YOLO postprocess still gates detections, while marker localization accepts
  # very low-confidence L/T/X candidates and lets the particle filter decide.
  export ROBOCUP_MARKER_SERVO_SCAN="${ROBOCUP_MARKER_SERVO_SCAN:-1}"
  export ROBOCUP_DETECT_MIN_SCORE="${ROBOCUP_DETECT_MIN_SCORE:-0.30}"
  export ROBOCUP_MARKER_MIN_CONFIDENCE="${ROBOCUP_MARKER_MIN_CONFIDENCE:-0.1}"
  ...
  export ROBOCUP_MARKER_FAR_CONFIDENCE_MAX="${ROBOCUP_MARKER_FAR_CONFIDENCE_MAX:-0.1}"
  ...
  export ROBOCUP_MARKER_INITIAL_CONFIRM="${ROBOCUP_MARKER_INITIAL_CONFIRM:-0}"
  ...
  export ROBOCUP_G1_BALL_FALLBACK_TTL_SEC="${ROBOCUP_G1_BALL_FALLBACK_TTL_SEC:-3}"
...
start_bg marker_locator \
  ...
  env LD_LIBRARY_PATH="$RUNTIME_LD_LIBRARY_PATH" ROBOCUP_MARKER_LOCATION_TOPIC="$ROBOCUP_MARKER_LOCATION_TOPIC" ROBOCUP_MARKER_PUBLISH_CONTINUOUS="$ROBOCUP_MARKER_PUBLISH_CONTINUOUS" ROBOCUP_MARKER_SERVO_SCAN="$ROBOCUP_MARKER_SERVO_SCAN" ./test_location "$IFACE" ../config.yaml "$MARKER_DISPLAY"
sleep 2
